...
puts "#{a.any? {|x| ((x[:mark1].to_i+x[:mark2].to_i)/2) >= 80}}, Shanta_taught_with_distinction."
puts "2) "
print "i) Teachers names are: " 
print school_hash.map { |x| x[:teacher] }.uniq
puts ''
print "ii) Students names are: "
print school_hash.map { |x| x[:student] }.uniq
puts ''
...
puts a = school_hash.any? { |x| x[:gender] == 'female' }
...
puts a.all? {|x| ((x[:mark1].to_i+x[:mark2].to_i)/2) >= 80} 
...
puts a = school_hash.any?{|x| ((x[:mark1].to_i+x[:mark2].to_i)/2) < 50 } 
